<?php
//$Id: msgParser.php,v 1.13 2005/02/15 16:58:03 ackbarr Exp $

if (!defined('MLDOCS_CONSTANTS_INCLUDED')) {
    exit();
}

define('MD5SIGNATUREPATTERN', '{([^ ]*)}');
define('HEADER_PRIORITY', 'Importance');
define('_MLDOCS_MSGTYPE_ARCHIVE', 1);
define('_MLDOCS_MSGTYPE_RESPONSE', 2);



require_once(MLDOCS_PEAR_PATH.'/Mail/mimeDecode.php');

/**
 * mldocsMessageParser class
 *
...
class mldocsEmailParser
{
    ...
    function &parseMessage(&$msg)
    {
        ...
        $newMsg  = new mldocsParsedMessage($struct);
    ...
    function &_parseMsg($msg)
    {
        ...
        $arr['hash'] = $this->_parseMd5Key($structure->headers['subject']);
    ...
    function _parseMd5Key($msg)
    {
        //check the email body for our md5 anchor
        eregi(MD5SIGNATUREPATTERN,$msg, $hash);
        if (count($hash)>0){
            return $hash[1];           
        }
    }
    ...
    {
        $pattern = '/^\*{4}\s'. _MLDOCS_ARCHIVE_MD5SIGNATURE . '\s(.)*\*{4}/im';
...
class mldocsParsedMessage
{
    ...
    function mldocsParsedMessage(&$msg)
    {
       ...
       $this->_msgtype     = (strlen($msg['hash']) == 0 ? _MLDOCS_MSGTYPE_ARCHIVE : _MLDOCS_MSGTYPE_RESPONSE);
