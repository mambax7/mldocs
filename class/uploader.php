...
// modification BYOOS solutions  le 20 avril 2006  gabriel
// ------------------------------------------------------------------------ //
...
class XoopsMediaUploader
{
    ...
    function fetchMedia($media_name, $index = null)
    {
      ...
      global $_FILES, $_xFILES, $FlagTypeUpload;
         //echo "entre dand la fonction fetchMedia<br>"; 
    	if (!$FlagTypeUpload){ // Upload manuel INPUT form
        	if (!isset($_FILES[$media_name]))
              ...
              if (!isset($_xFILES[$media_name]))
              {
                  $this->setErrors('You either did not choose a file to upload or the server has insufficient read/writes to upload this file.!');
                  return false;
              } else
              {
                  $media_name = @$_xFILES[$media_name];
                  $this->mediaName = (get_magic_quotes_gpc()) ? stripslashes($media_name['name']) : $media_name['name'];
        ...
        // test si le fichier est en place pour l' UPLOAD  FORM  ou BANNETTE
        $FlagFileExist = false;
        
        if ($FlagTypeUpload){
            if(file_exists($this->mediaTmpName)){
            $FlagFileExist = true;
            }
        } else {
    ...
    {
      //echo "entre dand la fonction Upload  OK<br>"; 
        if ($this->uploadDir == '')
        {
    ...
    {
      //echo "entre dand la fonction copyFile<br>"; 
        global $FlagTypeUpload; // upload manuel ou automatique
        $matched = array();
       ...
       // copy ou déplace le fichier en fonction du mode FORM ou BANNETTE
        if ($FlagTypeUpload){
