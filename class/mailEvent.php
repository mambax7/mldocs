 ...
 * @package mldocs
 */ 
class mldocsMailEvent extends XoopsObject {
    function mldocsMailEvent($id = null) 
	{
...
/**
 * mldocsMailEventHandler class
 *
 * MailEvent Handler for mldocsMailEvent class
 *
 * @author Eric Juden <ericj@epcusa.com> &
 * @access public
 * @package mldocs
...
class mldocsMailEventHandler extends mldocsBaseObjectHandler {
	/**
     * Name of child class
     * 
     * @var	string
     ...
     */	
	function mldocsMailEventHandler(&$db)
	{
	    parent::init($db);
    }
    ...
    function _selectQuery($criteria = null, $join = false)
    {
        if(!$join){
	        ...
	        $sql = sprintf("SELECT e.* FROM %s e INNER JOIN %s d ON d.id = e.mbox_id", $this->_db->prefix('mldocs_mailevent'), $this->_db->prefix('mldocs_department_mailbox')); 
	    }
	 ...
	 * @return array array of {@link mldocsMailEvent} objects
	 * @access	public	
	 */	
