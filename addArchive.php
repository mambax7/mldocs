        ...
        if($xoopsModuleConfig['mldocs_allowUpload']){
            ...
            $aMimetype = array();
             foreach($mimetypes as $key=>$mime){
              //traite la liste des extention de fichier autorisées
                if($mimes == ''){
            ...
            // retrouve le type MIME pour chaque extention de fichier autorisé
            foreach($UploadxFiles as $key=>$aFile){
                $UploadxFiles [$key] = $aFile;

                  // prépare la liste avant envoi vers le template  
                  if($ListeF == ''){
                            $ListeF = $aFile ['name'];
                     } else {                                      
                    $ListeF .= " ../.. " . $aFile ['name'];
            ...
            $pos =  array_search($dept_id, $aDept); //retrouve le département par défaut
...
//-->
    ...
    } else { // après le POST du formulaire
        
        if (MLDOCS_UNIQUE_ARCHIVE){// si le code archive est réglé sur unique, verifier si il n existe pas déjà
        ...
        $FlagTypeUpload= false;// UPload en manuel par défaut
        ...
        $archive->setVar('repid', $repid); // position dans le rayonnage répertoires disque
        ...
        $aUploadFiles = array();// fichiers transférés en manuel FORM
        $aUploadxFiles = array();// fichiers transférés en automatique BANNETTE
