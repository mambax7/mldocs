<?php
//$Id: fields.php,v 1.7 2005/08/23 16:16:40 eric_juden Exp $
    ...
    if (!isset($_POST['addField'])) {
        ...
        if ($count) {
            ...
            echo "<table width='100%' cellspacing='1' class='outer'>
                <tr><th colspan='7'><label>"._AM_MLDOCS_TEXT_MANAGE_FIELDS."</label></th></tr>";
            echo "<tr class='head'>
            ...
            foreach ($fields as $field) {
                ...
                $edit_url = mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php',
                    array('op' => 'editfield', 'id' => $field->getVar('id')));
                $del_url  = mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php',
                ...
                echo "<tr class='even'><td>".$field->getVar('id')."</td>
                    ...
                    <td>".mldocsGetControlLabel($field->getVar('controltype'))."</td>
                    <td><a href='".mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php', $req_link_params)."' title='$req_title'>$req_img</a></td>
        ...
        $field_info = $session->get('mldocs_addField');
        $field_errors = $session->get('mldocs_addFieldErrors');

        $hDepts =& mldocsGetHandler('department');
        ...
        if (! $field_errors === false) {
            mldocsRenderErrors($field_errors, mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php', array('op'=>'clearAddSession')));
        }
        ...
        $dept_select =& new XoopsFormSelect(_AM_MLDOCS_TEXT_DEPARTMENTS, 'fld_departments', $fld_departments, 5, true);        
        foreach($depts as $obj) {
        ...
        $form = new mldocsForm(_AM_MLDOCS_ADD_FIELD, 'add_field', mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php', array('op'=>'managefields')));
        ...
        $fieldnameEle = new XoopsFormText(_AM_MLDOCS_TEXT_FIELDNAME, 'fld_fieldname', 30, 64, $fld_fieldname);
        $fieldnameEle->setDescription(_AM_MLDOCS_TEXT_FIELDNAME_DESC);
        ...
        $dept_select->setDescription(_AM_MLDOCS_TEXT_DEPT_DESC);
        ...
        $btn_tray->addElement(new XoopsFormButton('', 'addField', _AM_MLDOCS_BUTTON_SUBMIT, 'submit'));
        ...
        $has_errors = false;
        $hField =& mldocsGetHandler('archiveField');
        ...
        if (trim($fld_fieldname) == '') {
            $has_errors = true;
            $errors['fld_fieldname'][] = _AM_MLDOCS_VALID_ERR_FIELDNAME;
        }
        ...
        if ($hField->getCount($crit)) {
            $has_errors = true;
            $errors['fld_fieldname'][] = _AM_MLDOCS_VALID_ERR_FIELDNAME_UNIQUE;
        }        
        ...
        if ($has_errors) {
            ...
            $session->set('mldocs_addField', $afield);
            $session->set('mldocs_addFieldErrors', $errors);
            header('Location: '. mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php'));
            exit();
        }
        ...
        if ($hField->insert($field)) {
            ...
            redirect_header(mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php'), 3, _AM_MLDOCS_MSG_FIELD_ADD_OK);
            
        } else {
            $errors = $field->getHtmlErrors();
            redirect_header(mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php'), 3, _AM_MLDOCS_MSG_FIELD_ADD_ERR . $errors);
        }
    }
}
...
function deleteField()
{
    ...
    if (!isset( $_REQUEST['id'])) {
        redirect_header(mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php', array('op'=> 'manageDepartments'), false), 3, _AM_MLDOCS_MESSAGE_NO_FIELD);
    }
    ...
    if (!isset($_POST['ok'])) {
        xoops_cp_header();
        echo $oAdminButton->renderButtons('manfields');
        xoops_confirm(array('op' => 'delfield', 'id' => $id, 'ok' => 1), MLDOCS_ADMIN_URL .'/fields.php', sprintf(_AM_MLDOCS_MSG_FIELD_DEL_CFRM, $id));
        xoops_cp_footer();
    } else {
        $hFields =& mldocsGetHandler('archiveField');
        ...
        if($hFields->delete($field, true)){
            ...
            header("Location: " . mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php', array('op'=>'manageFields'), false));
            exit();
            
        } else {
            redirect_header(mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php', array('op' => 'manageFields'), false), 3, $message);
        }
    }       
}
...
function editField()
{
    ...
    if (!isset($_POST ['editField'])) {
        //Get Custom Field From session (if exists)
        $field_info = $session->get('mldocs_editField_'.$fld_id);
        $field_errors = $session->get('mldocs_editFieldErrors_'.$fld_id);
        ...
        if (! $field_errors === false) {
            mldocsRenderErrors($field_errors, mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php', array('op'=>'clearEditSession', 'id'=>$fld_id)));
        }
        
        $form = new mldocsForm(_AM_MLDOCS_EDIT_FIELD, 'edit_field', mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php', array('op'=>'editfield', 'id'=>$fld_id)));
        ...
        $btn_tray = new XoopsFormElementTray('');
        $btn_tray->addElement(new XoopsFormButton('', 'editField', _AM_MLDOCS_BUTTON_SUBMIT, 'submit'));
        ...
        if ($has_errors) {
            ...
            $afield['length'] = $_POST['fld_length'];
            $afield['validation'] = ($_POST['fld_valid_select'] == $_POST['fld_valid_txtbox'] ? $_POST['fld_valid_select'] : $_POST['fld_valid_txtbox']);
            $session->set('mldocs_editField_'.$fld_id, $afield);
            $session->set('mldocs_editFieldErrors_'.$fld_id, $errors);
            //Redirect to edit page (display errors);
            header('Location: '. mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php', array('op'=>'editfield', 'id'=>$fld_id), false));
            exit();
        ...
        if ($hField->insert($field)) {
            ...
            redirect_header(mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php'), 3, _AM_MLDOCS_MSG_FIELD_UPD_OK);
        } else {
            redirect_header(mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php', array('op'=>'editfield', 'id'=>$fld_id), false), 3, _AM_MLDOCS_MSG_FIELD_UPD_ERR);
        }
    }  
...
function setFieldRequired()
{
    ...
    if ($field =& $hField->get($id)) {
        ...
        if ($ret) {
            ...
            redirect_header(mldocsMakeURI(MLDOCS_ADMIN_URL .'/fields.php'), 3, _AM_MLDOCS_MSG_FIELD_UPD_ERR);
        }
    } else {
        redirect_header(mldocsMakeURI(MLDOCS_ADMIN_URL .'/fields.php'), 3, _AM_MLDOCS_MESSAGE_NO_FIELD);
    }
...
function clearEditSession()
{
    ...
    header('Location: ' . mldocsMakeURI(MLDOCS_ADMIN_URL.'/fields.php', array('op'=>'editfield', 'id'=>$fieldid), false));
}

function _clearAddSessionVars()
{
    $session = Session::singleton();
    $session->del('mldocs_addField');
    $session->del('mldocs_addFieldErrors');
}
...
function _clearEditSessionVars($id)
{
    $id = intval($id);
    $session = Session::singleton();
    $session->del("mldocs_editField_$id");
    $session->del("mldocs_editFieldErrors_$id");
}
