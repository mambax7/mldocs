...
$_eventsrv->advise('delete_staff', mldocs_staffService::singleton());
...
if(isset($_REQUEST['deleteDept'])){
    if (isset( $_REQUEST['deptid'])){
        ...
        redirect_header(MLDOCS_ADMIN_URL."/department.php?op=manageDepartments", 3, _AM_MLDOCS_MESSAGE_NO_DEPT);
    }
    
    if (!isset($_POST['ok'])) {
        xoops_cp_header();
        echo $oAdminButton->renderButtons('manDept');
        xoops_confirm(array('deleteDept' => 1, 'deptid' => $deptID, 'ok' => 1), MLDOCS_BASE_URL .'/admin/delete.php', sprintf(_AM_MLDOCS_MSG_DEPT_DEL_CFRM, $deptID));
        xoops_cp_footer();
    } else {
        ...
        $crit = new CriteriaCompo(new Criteria('gperm_name', _MLDOCS_GROUP_PERM_DEPT));
        ...
        if($hDepartments->delete($dept)){
            ...
            $message = _MLDOCS_MESSAGE_DEPT_DELETE;
            ...
            $message = _MLDOCS_MESSAGE_DEPT_DELETE_ERROR . $dept->getHtmlErrors();
        }
    ...
    if (isset( $_REQUEST['uid'])){
        ...
        if (!isset($_POST['ok'])) {
            xoops_cp_header();
            echo $oAdminButton->renderButtons('manDept');
            xoops_confirm(array('deleteStaff' => 1, 'uid' => $staffid, 'ok' => 1), MLDOCS_BASE_URL .'/admin/delete.php', sprintf(_AM_MLDOCS_MSG_STAFF_DEL_CFRM, $staffid));
            xoops_cp_footer();
        } else {
            ...
            if($hStaff->delete($staff)){
                ...
                $message = _MLDOCS_MESSAGE_STAFF_DELETE;
            } else {
                $message = _MLDOCS_MESSAGE_STAFF_DELETE_ERROR . $staff->getHtmlErrors();
            }
