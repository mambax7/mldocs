...
$aSortBy = array('id' => _AM_MLDOCS_TEXT_ID, 'department' => _AM_MLDOCS_TEXT_DEPARTMENT);
...
function activateMailbox()
{
    ...
    $hMailbox =& mldocsGetHandler('departmentMailBox');
    if ($mailbox =& $hMailbox->get($id)) {
        $url = MLDOCS_BASE_URL.'/admin/department.php?op=editDepartment&id='. $mailbox->getVar('departmentid');
        ...
        redirect_header(MLDOCS_BASE_URL.'/admin/department.php?op=manageDepartments', 3, _MLDOCS_NO_MAILBOX_ERROR);
    }   
}
...
function addDepartmentServer()
{
  if(isset($_GET['id'])){
    ...
    redirect_header(MLDOCS_ADMIN_URL."/department.php?op=manageDepartments", 3, _AM_MLDOCS_DEPARTMENT_NO_ID);
  }


  $hDeptServers =& mldocsGetHandler('departmentMailBox');
  ...
  if ($hDeptServers->insert($server))   {
    header("Location: ".MLDOCS_ADMIN_URL."/department.php?op=manageDepartments");
  } else {
    redirect_header(MLDOCS_ADMIN_URL.'/department.php?op=manageDepartments', 3, _AM_MLDOCS_DEPARTMENT_SERVER_ERROR);
  }
}
...
function DeleteDepartmentServer() {
    ...
    if(isset($_REQUEST['id'])){
        ...
        redirect_header(MLDOCS_ADMIN_URL.'/department.php?op=manageDepartments', 3, _AM_MLDOCS_DEPARTMENT_SERVER_NO_ID);
    }
    ...
    if (!isset($_POST['ok'])) {
        ...
        xoops_confirm(array('op' => 'DeleteDepartmentServer', 'id' => $emailID, 'ok' => 1), MLDOCS_BASE_URL .'/admin/department.php', sprintf(_AM_MLDOCS_MSG_DEPT_MBOX_DEL_CFRM, $server->getVar('emailaddress')));
        xoops_cp_footer();     
    } else {
        //get handler
        if ($hDeptServers->delete($server,true)) {
            ...
            redirect_header(MLDOCS_ADMIN_URL.'/department.php?op=manageDepartments', 3, _AM_MLDOCS_DEPARTMENT_SERVER_DELETE_ERROR);
        }
    }
}
...
function deleteStaffDept()
{
    if(isset($_GET['deptid'])){
        ...
        redirect_header(MLDOCS_ADMIN_URL."/department.php?op=manageDepartments", 3, _AM_MLDOCS_MSG_NO_DEPTID);
    }
    if(isset($_GET['uid'])){
        ...
        redirect_header(MLDOCS_ADMIN_URL."/department.php?op=editDepartment&deptid=$deptID", 3, _AM_MLDOCS_MSG_NO_UID);
    }
    ...
    if($ret){
        header("Location: ".MLDOCS_ADMIN_URL."/department.php?op=editDepartment&deptid=$deptID");
    } else {
        redirect_header(MLDOCS_ADMIN_URL."/department.php??op=editDepartment&deptid=$deptID", 3, _AM_MLDOCS_MSG_REMOVE_STAFF_DEPT_ERR);
    }
}
...
function editDepartment()
{
    ...
    if(isset($_POST['updateDept'])){
        ...
        if (trim($_POST['newDept']) == '') {
            $hasErrors = true;
            $errors['newDept'][] = _AM_MLDOCS_MESSAGE_NO_DEPT;
        } else {
            ...
            if($existingDepts = $hDepartments->getCount($crit)){
                $hasErrors = true;
                $errors['newDept'][] = _MLDOCS_MESSAGE_DEPT_EXISTS;
        ...
        if ($hasErrors) {
            ...
            $aDept['tagSubject'] = $_POST['tagSubject'];
            $aDept['tagDescription'] = $_POST['tagDescription'];
            $aDept['groups'] = $groups;
            $session->set("mldocs_editDepartment_$deptID", $aDept);
            $session->set("mldocs_editDepartmentErrors_$deptID", $errors);
            header('Location: '. mldocsMakeURI(MLDOCS_ADMIN_URL.'/department.php', array('op'=>'editDepartment', 'deptid'=>$deptID), false));
            exit();
        }        
        ...
        $dept->setVar('tagSubject', $_POST['tagSubject']);
        $dept->setVar('tagDescription', $_POST['tagDescription']);
        ...
        if($hDepartments->insert($dept)){
            ...
            if(isset($_POST['defaultDept']) && ($_POST['defaultDept'] == 1)){
                mldocsSetMeta("default_department", $dept->getVar('id'));
            } else {
                ...
                mldocsSetMeta("default_department", $aDepts[0]);
            }
            ...
            if(count($confOption) > 0){
                ...
                if(!$hConfigOption->insert($confOption[0])){
                    redirect_header(MLDOCS_ADMIN_URL."/department.php?op=manageDepartments", 3, _AM_MLDOCS_MSG_UPDATE_CONFIG_ERR);
                }
            }
            _clearEditSessionVars($deptID);
            header("Location: ".MLDOCS_ADMIN_URL."/department.php?op=manageDepartments");
        } else {
            $message = _MLDOCS_MESSAGE_UPDATE_DEPT_ERROR . $dept->getHtmlErrors();
            redirect_header(MLDOCS_ADMIN_URL."/department.php?op=manageDepartments", 3, $message);
        }
        ...
        $session =& Session::singleton();
        $sess_dept = $session->get("mldocs_editDepartment_$deptID");
        $sess_errors = $session->get("mldocs_editDepartmentErrors_$deptID");
        
        //Display any form errors
        if (! $sess_errors === false) {
            mldocsRenderErrors($sess_errors, mldocsMakeURI(MLDOCS_ADMIN_URL.'/department.php', array('op'=>'clearEditSession', 'deptid'=>$deptID)));
        }
        ...
        if (! $sess_dept === false) {
            ...
            $fld_tagSubject = $sess_dept['tagSubject'];
            $fld_tagDescription = $sess_dept['tagDescription'];
            $fld_groups  = $sess_dept['groups'];
        } else {
            $fld_newDept = $dept->getVar('department');
            $fld_tagSubject = $dept->getVar('tagSubject');
            $fld_tagDescription = $dept->getVar('tagDescription');
            $fld_groups = $aPerms;
        }
        ...
        $form = new mldocsForm(_AM_MLDOCS_EDIT_DEPARTMENT, 'edit_dept', mldocsMakeURI(MLDOCS_ADMIN_URL.'/department.php', array('op'=>'editDepartment', 'deptid' => $deptID)));
        $dept_name =& new XoopsFormText(_AM_MLDOCS_TEXT_EDIT_DEPT, 'newDept', 20, 35, $fld_newDept);
        $dept_subject =& new XoopsFormText(_AM_MLDOCS_TEXT_EDIT_SUBJECT, 'tagSubject', 20, 35, $fld_tagSubject);
        $dept_description =& new XoopsFormText(_AM_MLDOCS_TEXT_EDIT_DESCRIPTION, 'tagDescription', 20, 35, $fld_tagDescription);
        ...
        $defaultDeptID = mldocsGetMeta("default_department");
        $defaultDept =& new mldocsFormCheckbox(_AM_MLDOCS_TEXT_DEFAULT_DEPT, 'defaultDept', (($defaultDeptID == $deptID) ? 1 : 0), 'defaultDept');
        $defaultDept->addOption(1, "");
        $btn_tray = new XoopsFormElementTray('');
        $btn_tray->addElement(new XoopsFormButton('', 'updateDept', _AM_MLDOCS_BUTTON_SUBMIT, 'submit')); 
        $form->addElement($dept_name);
        $form->addElement($group_select);
        $form->addElement($dept_subject);
        $form->addElement($dept_description);
        ...
        echo "<form action='".MLDOCS_ADMIN_URL."/department.php?op=deleteStaffDept&amp;deptid=".$deptID."' method='post'>";
        ...
        if($staffCount > 0){
            ...
            echo "    <td>
                          ...
                          <a href='".MLDOCS_ADMIN_URL."/department.php?op=deleteStaffDept&amp;uid=".$uid."&amp;deptid=".$deptID."'><img src='".XOOPS_URL."/modules/mldocs/images/button_delete.png' title='"._AM_MLDOCS_TEXT_DELETE_STAFF_DEPT."' name='deleteStaffDept' /></a>
                      </td>
                  </tr>";
            }
            echo "<tr>
        ...
        if (count($deptServers) > 0) {
          ...
          foreach($deptServers as $server){
            if ($server->getVar('active')) {
                $activ_link = '".MLDOCS_ADMIN_URL."/department.php?op=activateMailbox&amp;setstate=0&amp;id='. $server->getVar('id');
                $activ_img = $imagearray['online'];
                $activ_title = _AM_MLDOCS_MESSAGE_DEACTIVATE;
            } else {
                $activ_link = '".MLDOCS_ADMIN_URL."/department.php?op=activateMailbox&amp;setstate=1&amp;id='. $server->getVar('id');
            ...
            echo '<tr class="even">
                   ...
                   <td> <a href="'. $activ_link.'" title="'. $activ_title.'">'. $activ_img.'</a>
                        <a href="'.MLDOCS_ADMIN_URL.'/department.php?op=EditDepartmentServer&amp;id='.$server->GetVar('id').'">'.$imagearray['editimg'].'</a>
                        <a href="'.MLDOCS_ADMIN_URL.'/department.php?op=DeleteDepartmentServer&amp;id='.$server->GetVar('id').'">'.$imagearray['deleteimg'].'</a>
        ...
        $formElements = array('type_select', 'server_text', 'port_text', 'username_text', 'pass_text', 'priority_radio',
                              'email_text', 'btn_tray');
        $form = new mldocsForm(_AM_MLDOCS_DEPARTMENT_ADD_SERVER, 'add_server', mldocsMakeURI(MLDOCS_ADMIN_URL.'/department.php', array('op'=>'AddDepartmentServer', 'id' => $deptID)));
        ...
        $submit_button =& new XoopsFormButton('', 'updateDept2', _AM_MLDOCS_BUTTON_SUBMIT, 'submit');
...
function EditDepartmentServer()
{
    if(isset($_GET['id'])){
        ...
        redirect_header(MLDOCS_ADMIN_URL."/department.php?op=manageDepartments", 3);       // TODO: Make message for no mbox_id
    }
    ...
    if(isset($_POST['updateMailbox'])){
        ...
        if($hDeptServers->insert($deptServer)){
            header("Location: ".MLDOCS_ADMIN_URL."/department.php?op=editDepartment&deptid=".$deptServer->getVar('departmentid'));
        } else {
            redirect_header(MLDOCS_ADMIN_URL."/department.php?op=editDepartment&deptid=".$deptServer->getVar('departmentid'),3);
        }
    } else {
        global $oAdminButton;
        xoops_cp_header();
...
function manageDepartments()
{
    ...
    if(isset($_POST['addDept'])){
        ...
        if ($hasErrors) {
            ...
            $aDept['tagDescription'] = $_POST['tagDescription'];
            $aDept['groups'] = $groups;
            $session->set('mldocs_addDepartment', $aDept);
            $session->set('mldocs_addDepartmentErrors', $errors);
            header('Location: '. mldocsMakeURI(MLDOCS_ADMIN_URL.'/department.php', array('op'=>'manageDepartments'), false));
            exit();
        }
        
        $department =& $hDepartments->create();
        $department->setVar('department', $_POST['newDept']);
        $department->setVar('tagSubject', $_POST['tagSubject']);
        $department->setVar('tagDescription', $_POST['tagDescription']);
        ...
        if($hDepartments->insert($department)){
            ...
            if(isset($_POST['defaultDept']) && ($_POST['defaultDept'] == 1)){
                mldocsSetMeta("default_department", $deptID);
            }
            ...
            if(count($config) > 0){
                ...
                if(!$hConfigOption->insert($newOption)){
                    redirect_header(MLDOCS_ADMIN_URL."/department.php?op=manageDepartments", 3, _AM_MLDOCS_MSG_ADD_CONFIG_ERR);
                }
            }
            _clearAddSessionVars();
            header("Location: ".MLDOCS_ADMIN_URL."/department.php?op=manageDepartments");
        } else {
            $message = _MLDOCS_MESSAGE_ADD_DEPT_ERROR . $department->getHtmlErrors();
        }
        ...
        */
        ...
        $session =& Session::singleton();
        $sess_dept = $session->get('mldocs_addDepartment');
        $sess_errors = $session->get('mldocs_addDepartmentErrors');
        
        //Display any form errors
        if (! $sess_errors === false) {
            mldocsRenderErrors($sess_errors, mldocsMakeURI(MLDOCS_ADMIN_URL.'/department.php', array('op'=>'clearAddSession'), false));
        }
        ...
        echo "<form method='post' action='".MLDOCS_ADMIN_URL."/department.php?op=manageDepartments'>";
        echo "<table width='100%' cellspacing='1' class='outer'>
              <tr><th colspan='2'><label for='newDept'>". _AM_MLDOCS_LINK_ADD_DEPT ." </label></th></tr>";
        ...
        echo "<tr><td class='head' width='20%' valign='top'>". _AM_MLDOCS_TEXT_SUBJECT ."</td><td class='even'>";
        echo "<input type='text' id='tagSubject' name='tagSubject' class='formButton' value='$fld_tagSubject' /></td></tr>";
        echo "<tr><td class='head' width='20%' valign='top'>"._AM_MLDOCS_TEXT_DESCRIPTION ."</td><td class='even'>";
        echo "<input type='text' id='tagDescription' name='tagDescription' class='formButton' value='$fld_tagDescription' /></td></tr>";
        ...
        echo "<tr><td class='foot' colspan='2'><input type='submit' name='addDept' value='"._AM_MLDOCS_BUTTON_SUBMIT."' class='formButton' /></td></tr>";
        echo "</table><br />";
        echo "</form>";
        if($total > 0){     // Make sure there are departments
            echo "<form action='". MLDOCS_ADMIN_URL."/department.php?op=manageDepartments' style='margin:0; padding:0;' method='post'>";
            echo "<table width='100%' cellspacing='1' class='outer'>";
            echo "<tr><td align='right'>"._AM_MLDOCS_TEXT_SORT_BY." 
                        ...
                        echo "</select>
                          <input type='submit' name='dept_sort' id='dept_sort' value='"._AM_MLDOCS_BUTTON_SUBMIT."' />
                      </td>
                  </tr>";
            echo "</table></form>";
            echo "<table width='100%' cellspacing='1' class='outer'>
            ...
            if(isset($departmentInfo)){
                $defaultDept = mldocsGetMeta("default_department");
                foreach($departmentInfo as $dept){
                    ...
                    if($dept->getVar('id') != $defaultDept){
                        echo "<td class='even' width='10%'><a href='".MLDOCS_ADMIN_URL."/department.php?op=updateDefault&amp;id=".$dept->getVar('id')."'><img src='".MLDOCS_IMAGE_URL."/off.png' alt='"._AM_MLDOCS_TEXT_MAKE_DEFAULT_DEPT."' title='"._AM_MLDOCS_TEXT_MAKE_DEFAULT_DEPT."' /></a></td>";
                    } else {
                        echo "<td class='even' width='10%'><img src='".MLDOCS_IMAGE_URL."/on.png'</td>";
                    }
                    //echo "<td class='even' width='10%'><img src='".MLDOCS_IMAGE_URL."/". (($dept->getVar('id') == $defaultDept) ? "on.png" : "off.png")."'</td>";
                    echo "<td class='even' width='70'><a href='".MLDOCS_ADMIN_URL."/department.php?op=editDepartment&amp;deptid=".$dept->getVar('id')."'><img src='".XOOPS_URL."/modules/mldocs/images/button_edit.png' title='"._AM_MLDOCS_TEXT_EDIT."' name='editDepartment' /></a>&nbsp;&nbsp;";
                    echo "<a href='".MLDOCS_ADMIN_URL."/delete.php?deleteDept=1&amp;deptid=".$dept->getVar('id')."'><img src='".XOOPS_URL."/modules/mldocs/images/button_delete.png' title='"._AM_MLDOCS_TEXT_DELETE."' name='deleteDepartment' /></a></td></tr>";
                }
...
{
    $session = Session::singleton();
    $session->del('mldocs_addDepartment');
    $session->del('mldocs_addDepartmentErrors');
}
...
function _clearEditSessionVars($id)
{
    $id = intval($id);
    $session = Session::singleton();
    $session->del("mldocs_editDepartment_$id");
    $session->del("mldocs_editDepartmentErrors_$id");
}

function updateDefault()
{
    $id = intval($_REQUEST['id']);
    mldocsSetMeta("default_department", $id);
    header('Location: '. mldocsMakeURI(MLDOCS_ADMIN_URL.'/department.php', array('op'=>'manageDepartments'), false));
}
?>
