    ...
    if(!isset($_POST['updateStaff'])){
        ...
        foreach($departmentInfo as $dept) {
            ...
            $checked = ($inDept ? "checked='checked'" : '');
				...
				//modif byoos le 2007/09/25   default department for user          
            $deptdef  = $staff->getVar('deptUserdef');    
            $checkedR = ($deptdef == $deptid ? "checked='checked'" : '');
            
            printf("<tr><td><input type='checkbox' name='departments[]' value='%u' %s onclick=\"mldocsRoleCustOnClick('frmEditStaff', 'departments[]', 'mldocs_depts', '&amp;', 'mldocs_dept_cust');\" />%s [<a href='staff.php?op=customDept&amp;deptid=%u&amp;uid=%u&amp;mldocs_role=%s&amp;mldocs_depts=%s' class='mldocs_dept_cust'>Customize</a>] <i>%s</i><input type='hidden' name='custrole[%u]' value='%s' /></td>", $deptid, $checked, $deptname, $deptid, $uid, $deptroleids, $mldocs_depts, $deptrolenames, $deptid, $deptroleids);
            printf("<td><input type='radio' name='departmentUserdef' value='%u' %s   /><td></tr>", $deptid, $checkedR);        
        }
        echo "<tr><td>
                  <input type='checkbox' name='checkAll' value='0' onclick='selectAll(this.form,\"departments[]\", this.checked);mldocsRoleCustOnClick(\"frmEditStaff\", \"departments[]\", \"mldocs_depts\", \"&amp;\", \"mldocs_dept_cust\");' /><b>"._AM_MLDOCS_TEXT_SELECT_ALL."</b></td>";
        echo "<td><input type='radio' name='departmentUserdef'".($deptdef == 0 ? "checked='checked'" : '')." value='0'  /><td><b>"._AM_MLDOCS_TEXT_SELECT_AUCUN."</b></td><td></tr>";
        echo "<tr><td>";
        echo "</td></tr>";
        echo "</table>";
        echo "</td></tr>";
        ...
        $custroles = $_POST['custrole'];
				//modif byoos le 2007/09/25    department by default for one user       
        $deptUserdef = isset ( $_POST['departmentUserdef'] ) ? $_POST['departmentUserdef'] : '0'; 
        ...
        $staff =& $hStaff->getByUid($uid);
        $staff->setVar('deptUserdef', $deptUserdef);    
