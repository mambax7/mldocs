var MLDOCS_CONTROL_TXTBOX = 0;
var MLDOCS_CONTROL_TXTAREA = 1;
var MLDOCS_CONTROL_SELECT = 2;
var MLDOCS_CONTROL_MULTISELECT = 3;
var MLDOCS_CONTROL_YESNO = 4;
var MLDOCS_CONTROL_CHECKBOX = 5;
var MLDOCS_CONTROL_RADIOBOX = 6;
var MLDOCS_CONTROL_DATETIME = 7;
var MLDOCS_CONTROL_FILE = 8;
...
function mldocsRoleCustOnClick(frmName, roleName, roleParam, joinChr, className)
{
    if (joinChr.length ==0) {
        joinChr = '&amp;';
    }
...
function mldocsPortOnChange(srvtype, portfld)
{
    fld = gE(portfld);
    switch(srvtype) {
    case 'POP3':
...
function mldocsDOMAddEvent(obj, evType, fn, useCapture){
...
function mldocsDOMRemoveEvent(obj, evType, fn, useCapture)
{
...
function mldocsFillStaffSelect(sel, values)
{
    if (values.length > 0) {
        var val = sel.value;
        var uid = '';
...
function mldocsFillSelect(sel, values)
{
    if (values.length > 0) {
        var val = sel.value;
        var myKey = '';
...
function mldocsFillCustomFlds(tbl, result, before)
{
    var flds = mldocsGetElementsByClassName(tbl, 'custfld_value');
    var values = Array();
    ...
    if(result.length > 0) {       
        for (n = 0; n < result.length; n++) {
            ...
            if (data != null) {
                ...
                inp = mldocsCreateInpField(controlType, fieldName, defaultValue, currentValue, fieldLength, values);
                tbl.insertBefore(mldocsCreateCustFldRow(name, desc, inp), before);
            }
...
function mldocsGetElementsByClassName(parentEle, className)
{
    var ele = Array();
    if (parentEle.hasChildNodes()) {
        ...
        for (var n = 0; n < chlds.length; n++) {
            ...
            if (chlds[n].hasChildNodes()) {
                ele.concat(mldocsGetElementsByClassName(chlds[n], className));
            }
...
function mldocsCreateInpField(controlType, fieldName, defaultValue, currentValue, fieldLength, values)
{   
    ...
    {
        case '0':
            ele = mldocsCreateTextBox(fieldName, value, length, fieldLength);
            break;
        case '8':
            ele = mldocsCreateFile(fieldName, length);
            break;
        case '1':
            ele = mldocsCreateTextArea(fieldName, value, length, 5);
            break;
        case '2':
        case '3':
            ele = mldocsCreateSelect(fieldName, values, value, 1, false);
            break;
        case '5':
            ele = mldocsCreateCheckbox(fieldName, values, value);
            break;
        case '6':
        case '4':
            ele = mldocsCreateRadiobox(fieldName, values, value);
            break;
        case '7':
            ele = mldocsCreateDateTime(fieldName, value);
            break;
    }
    return ele; 
}

function mldocsCreateTextArea(fieldName, value, cols, rows)
{
...
function mldocsCreateTextBox(fieldName, value, length, maxLength)
{
...
function mldocsCreateFile(fieldName, length)
{
    var l = cE(document.all?'<input name="'+fieldName+'">':'input');
...
function mldocsCreateSelect(fieldName, values, value, size, multiple)
{
...
function mldocsCreateCheckbox(fieldName, values, value)
{
    var ele = Array();
    var l = null;
    for (var n=0; n<values.length; n++) {
        ...
        l.value = values[n][0];
        if (isArray(value)) {
            l.checked = mldocsInArray(l.value, value);
        } else {
...
function mldocsCreateRadiobox(fieldName, values, value)
{
    var ele = Array();
    var l = null;
    for (var n=0; n<values.length; n++) {
...
function mldocsCreateDateTime(fieldName, value)
{    
    var l = mldocsCreateTextBox(fieldName, value, 50, 25);
    return l;
}

function mldocsCreateCustFldRow(name, description, ele)
{
    var tr = cE('tr');
    var td1 = cE('td');
    var td2 = cE('td');
...
function mldocsInArray(needle, haystack)
{
