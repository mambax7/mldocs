...
function mldocsRenderErrors(&$err_arr, $reseturl = '')
{

    if (is_array($err_arr) && count($err_arr) > 0) {
        echo '<div id="readOnly" class="errorMsg" style="border:1px solid #D24D00; background:#FEFECC url('. MLDOCS_IMAGE_URL.'/important-32.png) no-repeat 7px 50%;color:#333;padding-left:45px;">';
    
        echo '<h4 style="text-align:left;margin:0; padding-top:0">'._AM_MLDOCS_MSG_SUBMISSION_ERR;
        
        if ($reseturl) {
            echo ' <a href="' . $reseturl . '">[' . _AM_MLDOCS_TEXT_SESSION_RESET . ']</a>';
        }
